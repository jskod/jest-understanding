...
### Pass `done` parameter to make it work as intended
...
### .resolves & .rejects
You can also use the .resolves matcher in your expect statement, and Jest will wait for that promise to resolve. If the promise is rejected, the test will automatically fail. It works similarly to `await` keyword.
...
### Async/Await
Alternatively, you can use async and await in your tests. Using `async/await` is more readable and concise and wins the race.

Above scenario can be written using `async/await` like:
